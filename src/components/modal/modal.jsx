...
const modal = ({ setIsShowModal, setClickedMovieInfo, clickedMovieInfo }) => {
  useEffect(() => {
    ...
    axios.get(url).then((response) => {
      const { genre, nation, plots: { plot }, rating, stlls } = response.data.Data[0].Result[0];
      clickedMovieInfo.actor = clickedMovieInfo.actor.split('|');
      clickedMovieInfo.director = clickedMovieInfo.director.split('|');
      setClickedMovieInfo({ ...clickedMovieInfo, genre, nation, plot: plot[0], rating, stlls: stlls.split('|') });
    });
  }, []);
  useEffect(() => {
